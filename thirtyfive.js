...
function showWinningNotification() {
    ...
    for (let i = 0; i < 35; i++) { //PARAM
        const tile = document.createElement('div');
        tile.id = 'tile' + (i + 1);
        tile.classList.add('tile');
        tile.textContent = (i + 1).toString();
        tile.style.left = `${(i % 6) * 100}px`; //PARAM
        tile.style.top = `${Math.floor(i / 6) * 100}px`; //PARAM
        tile.style.backgroundImage = 'url(background1.png)'; //PARAM
        tile.style.backgroundPosition = `-${(i % 6) * 100}px -${Math.floor(i / 6) * 100}px`; //PARAM
