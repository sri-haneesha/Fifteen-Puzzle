window.addEventListener('DOMContentLoaded', (event) => {
    ...
    const emptySpace = { x: 1, y: 1 };  // Bottom right corner as the empty space //PARAM
    let tiles = [];
    ...
    for (let i = 0; i < 3; i++) { 
        ...
        tile.style.left = `${(i % 2) * 100}px`;
        tile.style.top = `${Math.floor(i / 2) * 100}px`;
        tile.style.backgroundImage = 'url(background3.jpeg)'; //PARAM
        tile.style.backgroundPosition = `-${(i % 2) * 100}px -${Math.floor(i / 2) * 100}px`;
